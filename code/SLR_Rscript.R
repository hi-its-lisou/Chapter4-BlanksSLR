...
#Plot it
fig2a <- (ggplot(summary_df, aes(x = as.factor(year), y = n, fill = interaction_level)) +
  ...
  scale_fill_manual(
    values = c(
      "Did not report using a negative control" = "#000000",
...
fig2a
ggsave("figures/Figure2a.png", height=6, width=9)
...
fig2b<- (neg_prop %>%
    ...
    geom_line(data = controlcontam_prop,
